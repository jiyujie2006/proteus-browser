...
- **M0 local subset ready:** the local verification ruler, fixture/conformance
  tests, patch metadata checks, and explicitly demo/stub build tooling run
  locally.
- **M0 hard exit not met:** the 17 Chromium patches are metadata-only
  placeholders, and no signed, independently reproducible Win/macOS/Linux
  Chromium bundles or builder-attested live verification reports exist. The
  current entrypoint evidence layer is explicitly below hard-exit assurance.
  The live audit is documented in [M0.md](../M0.md).
...
**Current status:** not complete. Only the local ruler/tooling scaffold is
runnable; none of the three-platform engine-artifact exit evidence exists.
...
**Work**
- Chromium fork + layered patch scaffold (layer0 de-google, empty layer1/2) that
  **builds** on all three platforms via depot_tools/gn/ninja/sccache.
...
- **Stealth CDP endpoint** + Playwright/Puppeteer adapters (tdd/04); Selenium if
  it fits.
